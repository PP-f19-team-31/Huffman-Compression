...
echo plot parallel with different size
for method in compress decompress;do
    for size in $(seq -f '%02g' 1 $FILENUM);do
        for d in chinese english binary;do
            gnuplot -e "figtitle='${method} on different size of ${d} files';
            t1='../performance/parallel1_${method}_${d}.txt';
...
for size in $(seq -f '%02g' 1 $FILENUM);do
    for d in chinese english binary;do
        ...
        for method in compress decompress;do
            ...
            t1=`cat ../performance/parallel1_${method}_${d}.txt | sed "$(echo $size | sed 's/^0*//')!d" | awk '{print $2}'`
            ...
            echo "${method}() $s $t1 $t4 $t8 $t16 $t32 $t64" >> data.txt
        done
        cat data.txt
